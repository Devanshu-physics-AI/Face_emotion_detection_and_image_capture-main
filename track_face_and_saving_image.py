...
import time
import os
print("On what expression you want to click the photos?")
print(["angry","disgust","fear","happy","neutral","sad","surprise"])
required_expression=input()
print("Number of images you want to click?")
no_images=int(input())
...
def resize(x):
    ...
    return img
current=0   
generator=os.walk("/users/devanshusharma/desktop/image_processing_images")
for a,b,files in generator:
   for file in files:
      initial=a
      os.remove(initial+"/"+file)

while(current<no_images):
...
# while True:
 ...
 if len(face)>0:
    for x,y,w,h in face:
        ...
        frame=cv2.putText(frame,text=expression,org=(x,y-2),fontFace=cv2.FONT_HERSHEY_SCRIPT_COMPLEX,thickness=1,color=(0,0,255),fontScale=2)
        print(expression)
        if(expression==required_expression):
         current+=1
         cv2.imwrite(f"/users/devanshusharma/desktop/image_processing_images/{current}.jpeg",frame)
        time.sleep(1)
